    ...
    while (settings.FAKE_STREAMING and (current_try_num < max_retry_num) and (empty_response_count < settings.MAX_EMPTY_RESPONSES)):
        ...
        while tasks and not success:
            ...
            if empty_response_count >= settings.MAX_EMPTY_RESPONSES:
                ...
                if is_gemini :
                    yield gemini_from_text(content="số lượng phản hồi trống đã đạt đến giới hạn\nvui lòng sửa đổi tham số hoặc nội dung prompt",finish_reason="STOP",stream=True)
                else:
                    yield openAI_from_text(model=chat_request.model,content="số lượng phản hồi trống đã đạt đến giới hạn\nvui lòng sửa đổi tham số hoặc nội dung prompt",finish_reason="stop",stream=True)
